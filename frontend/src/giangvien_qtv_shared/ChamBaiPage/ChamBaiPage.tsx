...
const ChamBaiPage = () => {
  ...
  useEffect(() => {
    if (!maBaiNop) return;
    fetch(`${(window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1" || window.location.hostname.startsWith("192.168.") || window.location.hostname.startsWith("172.") || window.location.hostname.startsWith("10.") ? "http://" + window.location.hostname + ":5004" : "http://14.225.192.252:5004")}/bainop/${maBaiNop}`)
      .then(res => res.json())
      .then(data => {
        setBaiNop(data);
        if (data.Diem !== null && data.Diem !== undefined) setDiem(data.Diem.toString());
        if (data.NhanXet) setNhanXet(data.NhanXet);
        return fetch(`${(window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1" || window.location.hostname.startsWith("192.168.") || window.location.hostname.startsWith("172.") || window.location.hostname.startsWith("10.") ? "http://" + window.location.hostname + ":5004" : "http://14.225.192.252:5004")}/baitap/${data.MaBaiTap}`);
      })
      .then(res => res.json())
  ...
  const handleCham = async () => {
    ...
    setLoading(true);
    try {
      await fetch(`${(window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1" || window.location.hostname.startsWith("192.168.") || window.location.hostname.startsWith("172.") || window.location.hostname.startsWith("10.") ? "http://" + window.location.hostname + ":5004" : "http://14.225.192.252:5004")}/bainop/${maBaiNop}/cham`, {
        method: "PUT",
    ...
    <div className="cb-wrapper">
      ...
      <div className="cb-card">
        ...
        {(isExam || hasSections) && parsedSubmission ? (
          ...
          /* ── FALLBACK CASE 3: OLD TEXT SUBMISSIONS ── */
          <div>
            {deiBai && (
