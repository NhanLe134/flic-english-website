...
const TaoBaiTap = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://14.225.192.252:5000/baitap/${editDraftId}`)
      .then(res => res.json())
      .then(data => {
        if (data) {
          ...
          if (data.MaBuoiHoc) {
            ...
            // Load lesson details
            fetch(`http://14.225.192.252:5000/buoihoc/${buoiHocId}`)
              .then(res => res.json())
  ...
  const handleCreate = async (statusOverride?: "draft" | "pending" | "published" | "practice") => {
    ...
    try {
      ...
      const requestUrl = editDraftId 
        ? `http://14.225.192.252:5000/baitap/${editDraftId}`
        : "http://14.225.192.252:5000/baitap/create";
